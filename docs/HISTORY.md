# Shipped history (archived from TODO.md)

Point-in-time records of completed work, kept for the embedded lessons
and "what NOT to do" notes. Current open tasks live in [TODO.md](TODO.md);
the running feature inventory in [PROGRESS.md](PROGRESS.md). Newest first.

---

## ✅ Highlight clip export + Preview Cut COMMITTED 2026-06-05 (`9b1624e`)

Three operator-facing additions around the Highlights panel + player.
Frontend-only except the one export endpoint.

- **Per-highlight clip export.** Small ⤓ button on each highlight row →
  `POST /api/highlights/export` cuts the raw source segment to
  `output/<project>_hl<NN>_<start>-<end>.mp4` (NVENC re-encode =
  frame-accurate; audio mapped iff present; camera timecode track
  dropped via `-write_tmcd 0`). Source resolved the same robust way as
  the renderer (`_resolve_export_source`: session token → else
  `video_file` as absolute path or `videos/` basename), so it works on
  reloaded projects after a server restart. Saves to `output/` only
  (operator dropped the browser-download leg). The three row actions
  (export / jump / delete) are grouped with wider spacing.
- **Preview Cut toggle.** Player's "▶ Preview cut" button plays only the
  kept segments, auto-jumping past every trim (manual + auto) on
  playback AND manual seeks — same set the render keeps. Scoreboard
  overlay stays correct (score events live in kept segments).
- **Preview Cut transport bar.** Dedicated control row under the main
  controls, visible only while Preview Cut is on (accent border +
  pulsing label, clear of the bottom-right scoreboard). Play/pause,
  ⏪/⏩, and the position readout all operate on KEPT time via
  source↔kept mapping; speed pills (0.5/1/1.5/2×) sync with the main
  speed selector.

Files: [routes_render.py](../backend/server/routes_render.py),
[highlights.js](../frontend/highlights.js),
[player.js](../frontend/player.js), [index.html](../frontend/index.html),
[state.js](../frontend/state.js). 170 pytest pass.
