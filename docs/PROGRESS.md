...
Last update: 2026-07-15 midday (G0B GATE MEASURED — **STOP-LOSS**: zero-shot 57.7% vs fine-tuned 48.1% on the 79-event pinned held-out; on val the adapter answers the majority class 49/49 times while train_loss 0.12 shows pure memorization → 13 matches are not enough for transferable winner-reading, same venue-overfit mode as the frame-CNN spike. Phase 2 auto-winner GUI is NOT justified; semi-auto Phase 1 stays the production flow. The measurement is trustworthy: label masking was verified to supervise exactly the 10 answer tokens after finding that the original collator left 1282/1569 tokens (image pads + prompt) supervised — the two earlier "training runs" were invalid. Training infra now solid and reusable: g0b_overnight.py driver, per-step heartbeat + 📊 Training dashboard section (live step speed / ETA / 🟢🟡🔴 health), 5-step checkpoints + shutdown-safe auto-resume, VRAM hard cap (spill → loud OOM), StepSpeedGuard, letterboxed frames (shape fragmentation was the overnight OOM root cause; peft's prepare_model_for_kbit fp32 upcast was the load-time OOM; expandable_segments is a no-op on Windows). Epoch-1 wall-clock 77 min at 40 s/step flat, VRAM 7.93 GiB flat. Tests: **298 pass**.)
Previous update: 2026-07-14 night (CORPUS 15/15 — G0b overnight pipeline RUNNING: corpus rebuild → truly-unseen eval on the 3 newest matches (v7-tuned2 coverage 95.9%, assoc 93.0% over 171 events; tonight's one-click-render match 100%/98%) → G0b dataset 485→~880 train + 79-event held-out eval → QLoRA fine-tune → verdict; log at temp/g0b_overnight.log. Same night, two launch bugs fixed: retro side-labels lived only in project.json while build_corpus.py read groundtruth.json (eval split silently empty — operator's labels were fine all along), and Qwen3.5 thinking-mode meant zero-shot generation never reached the JSON within the token budget (eval now renders prompts with enable_thinking=False, matching the training collator's supervised region exactly). FAIL-LOUD VALIDATION AUDIT repo-wide (operator directive): every pipeline step now exits non-zero on empty/degenerate data and every UI failure path toasts — confirm_roi quarantines corrupt label files instead of wiping history, rally detection raises on mid-stream ffmpeg death instead of "succeeding" over half a match, keep-the-winner rolls back on inconclusive comparisons, compare_roi_models refuses all-nan verdicts, pollRender un-sticks itself after 8 failed polls, training dashboard distinguishes corrupt-manifest/unreadable-snapshot from empty/unlabeled. Tests: **293 pass**. One-click render chain operator-live-tested OK in production (48 auto trims / 0 manual). Earlier same day — G0B PIPELINE PREPPED: build_g0b_dataset / train_g0b_lora (QLoRA, Qwen/Qwen3.5-9B verified on HF) / eval_g0b with PASS/MARGINAL/STOP-LOSS verdict — milestone day at 15 labeled matches is now execution-only; requirements-g0b.txt keeps the training stack out of CI. Truly-unseen segmentation eval on the 6 new production matches: v7-tuned2 coverage 93.7% (340/363), assoc 91.7% — G0a holds in production. ONE-CLICK RENDER: the Render button now guarantees auto trims exist before ffmpeg starts — missing trims trigger headless rally detection (+ the Auto Trim modal for ROI confirm when needed) and auto-apply, then render; the modal also gets a "⚡ Detect + Apply + Render" combo button. `frontend/render_chain.js`; E2E 12/12 with a real 193 s detection. Avatar name type-ahead: p1-p4 inputs suggest from the photo roster with accent-insensitive search + thumbnail rows — GET /api/avatars + frontend/avatar_suggest.js, E2E 10/10. Tests: **289 pass**. Note-suffix avatar rule: a trailing "(note)" on a player name — `Lương Đức Tuấn (Gai Dài)` — now stays on the scoreboard/intro as typed while avatar lookup + doubles name-combine ignore it; `strip_note_suffix` in ass/common.py mirrored in app.js. Tests: **287 pass**. Intro preview: "👁 Preview intro" in the Render panel renders just the ~4 s intro through the production code path — cinematic cold 4.4 s, cache hit 120 ms — so title overflow / missing avatars surface in seconds, not after a full render. Tests: **281 pass**. Same day: keep-the-winner retrain policy: the 07-13 GUI retrain A/B'd REGRESSED, so the previous weights were restored and `retrain.py` now auto-rolls back to `roi_seg.prev.pt` on any REGRESSED verdict — retrain frequency can no longer hurt the production model; verified E2E with a real retrain reproducing the regression. Dead-artifact cleanup: obsolete YOLO run dirs + ~3 GB of failed-render temp leftovers deleted, and `prune_stale_job_dirs` now age-gates (7 d) failed-render temp/<job_id> dirs at every server start. Plus status sync: production flywheel running — 6 new manual-production matches archived 07-10..07-13 with side-info + handicap labels captured at production time → **corpus 12/15 labeled** toward the G0b target).
